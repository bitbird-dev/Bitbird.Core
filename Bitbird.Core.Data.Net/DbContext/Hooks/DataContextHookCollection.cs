...
namespace Bitbird.Core.Data.Net.DbContext.Hooks
{
        ...
        where TDataContext : System.Data.Entity.DbContext
    {
        ...
        internal Task InvokePreInsertAsync(TDataContext db, TState state, EntityHookEvent[] entities) => VisitHookEventsAsync(entities, async (hookEvents, typedEntities) => await hookEvents.InvokePreInsertAsync(db, state, typedEntities));
        internal Task InvokePostInsertAsync(TDataContext db, TState state, EntityHookEvent[] entities) => VisitHookEventsAsync(entities, async (hookEvents, typedEntities) => await hookEvents.InvokePostInsertAsync(db, state, typedEntities));
        internal Task InvokePreDeleteAsync(TDataContext db, TState state, EntityHookEvent[] entities) => VisitHookEventsAsync(entities, async (hookEvents, typedEntities) => await hookEvents.InvokePreDeleteAsync(db, state, typedEntities));
        internal Task InvokePostDeleteAsync(TDataContext db, TState state, EntityHookEvent[] entities) => VisitHookEventsAsync(entities, async (hookEvents, typedEntities) => await hookEvents.InvokePostDeleteAsync(db, state, typedEntities));
        internal Task InvokePreUpdateAsync(TDataContext db, TState state, EntityHookEvent[] entitiesHookEvent) => VisitHookEventsAsync(entitiesHookEvent, async (hookEvents, typedEntities) => await hookEvents.InvokePreUpdateAsync(db, state, typedEntities));
        internal Task InvokePostUpdateAsync(TDataContext db, TState state, EntityHookEvent[] entitiesHookEvent) => VisitHookEventsAsync(entitiesHookEvent, async (hookEvents, typedEntities) => await hookEvents.InvokePostUpdateAsync(db, state, typedEntities));
    }
}