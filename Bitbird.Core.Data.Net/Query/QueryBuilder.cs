...
using Bitbird.Core.Data.Net.Query.Mapping;
...
namespace Bitbird.Core.Data.Net.Query
{
    ...
    public static class QueryBuilder
    {
        ...
        private static readonly MethodInfo LikeMethod = typeof(DbFunctions).GetMethods().Single(method => method.Name == nameof(DbFunctions.Like) && method.GetParameters().Length == 2);
        ...
        /// <returns>The sorted query.</returns>
        internal static IOrderedQueryable<TModel> BuildSortedQuery<TModel>(this IQueryable<TModel> query, QuerySortProperty[] sortProperties)
        {
        ...
        /// <param name="sortProperties">Sorting information. Can be null (if null, no sorting will be applied). Must not contain null entries.</param>
        /// <returns>The sorted query.</returns>
        internal static IOrderedQueryable<TDbModel> BuildSortedQuery<TDbModel, TModel>(this IQueryable<TDbModel> query, QuerySortProperty[] sortProperties)
        {
            if (sortProperties == null || sortProperties.Length == 0)
            {
        ...
        internal static IQueryable<TModel> BuildFilteredQuery<TModel>(this IQueryable<TModel> query, QueryFilter[] filters)
        {
                ...
                switch (filter)
                {
                    ...
                    case QueryFreeTextFilter freeText:
                        ...
                        if (query.IsInMemory())
                            ...
                            currentFilterExpression = Expression.Call(null, LikeMethod, lambdaUnderlyingBody, Expression.Constant(freeText.Pattern.Replace("*", "%")));
                        break;
                    default:
        ...
        internal static IQueryable<TDbModel> BuildFilteredQuery<TDbModel, TModel>(this IQueryable<TDbModel> query, QueryFilter[] filters)
        {
            if (filters == null || filters.Length == 0)
                return query;
                ...
                switch (filter)
                {
                    ...
                    case QueryFreeTextFilter freeText:
                        if (lambdaData.UnderlyingDbModelType != typeof(string))
                            throw new NotSupportedException("Freetext filters are only allowed on string fields.");
                        currentFilterExpression = Expression.Call(null, LikeMethod, lambdaData.UnderlyingExpression, Expression.Constant(freeText.Pattern.Replace("*","%")));
                        break;
                    default:
                        throw new NotSupportedException($"QueryBuilder.BuildFilteredQuery: A {nameof(QueryFilter)} of type {filter.GetType().Name} was found, but is currently not supported");
                }
        ...
        {
            var dataQuery = query.BuildSecuredQuery(permissionResolver);
            var pagedQuery = dataQuery;

            if (queryInfo != null)
            {
                dataQuery = dataQuery.BuildFilteredQuery<TDbModel, TModel>(queryInfo.Filters);
                pagedQuery = dataQuery.BuildSortedQuery<TDbModel, TModel>(queryInfo.SortProperties)
                                      .BuildPagedQuery(queryInfo.Paging);
            }
            ...
            where TModel : class
        {
            ...
            if (queryInfo != null)
            {
                dataQuery = dataQuery.BuildFilteredQuery(queryInfo.Filters);
                pagedQuery = dataQuery.BuildSortedQuery(queryInfo.SortProperties)
                                      .BuildPagedQuery(queryInfo.Paging);
            }
        ...
        public static async Task<QueryResult<TModel>> ExecuteAsync<TModel>(this BuiltQuery<TModel> query, BenchmarkSection benchmarks = null)
            where TModel : class
        {
            TModel[] data;
            using (benchmarks.CreateBenchmark("QueryData"))
            {
                data = await query.PagedQuery.ToArrayAsync();
            }

            long count;
            using (benchmarks.CreateBenchmark("QueryCount"))
            {
                count = await query.Query.LongCountAsync();
            }
